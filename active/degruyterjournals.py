...
if not os.path.isfile('/tmp/dg%s' % (jnlfilename)):
    ...
    print tocurl
    os.system('wget -T 300 -t 3 -O /tmp/dg%s -U "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:39.0) Gecko/20100101 Firefox/39.0"  %s' % (jnlfilename, tocurl))
    ...
    for a in h4.find_all('a'):
        if a.has_attr('href'):
            ...
            if re.search('view\/', a['href']):
                ...
                recs.append(rec)
                print '  ', rec.keys()
            elif re.search('document\/doi', a['href']):
                for meta in artpage.find_all('meta'):
                    if meta.has_attr('name'):
                        #authors
                        ...
                        #date
                        elif meta['property'] == 'article:published_time':
