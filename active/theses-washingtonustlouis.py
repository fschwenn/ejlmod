# -*- coding: utf-8 -*-
#harvest theses from Washington U., St. Louis
#FS: 2021-11-19
...
publisher = 'Washington U., St. Louis'
...
uninteresting += [re.compile('Nutrition'), re.compile('Accounting'), re.compile('Anthropology'),
                  ...
                  re.compile('Movement Science'), re.compile('East Asian'),
                  re.compile('Political'), re.compile('Environmental'), re.compile('French')]

hdr = {'User-Agent' : 'Magic Browser'}
prerecs = []
jnlfilename = 'THESES-WashingtonUStLouis-%s' % (stampoftoday)
for i in range(numofpages):
    if i == 0:
        tocurl = 'https://openscholarship.wustl.edu/art_sci_etds/index.html'
    else:
        tocurl = 'https://openscholarship.wustl.edu/art_sci_etds/index.%i.html' % (i+1)
...
for rec in prerecs:
    ...
    #embargo
    for div in artpage.find_all('div', attrs = {'class' : 'aside'}):
        for p in div.find_all('p', attrs = {'class' : 'beta-embargo'}):
            embargo = True
            ...
            if meta['name'] == 'bepress_citation_author':
                ...
                for div in artpage.body.find_all('div', attrs = {'id' : 'orcid'}):
                    for p in div.find_all('p'):
                        rec['autaff'][-1].append('ORCID:%s' % (re.sub('.*org\/', '', p.text.strip())))
    ...
    if not 'doi' in rec.keys():
        rec['doi'] = '30.3000/WashingtonUStLouis/' + re.sub('\D', '', rec['artlink'])
    ...
    #Department
    if not skipit:
        for div in artpage.body.find_all('div', attrs = {'id' : 'authors_dept'}):
            for p in div.find_all('p'):
                dep = p.text.strip()
                rec['note'].append(dep)
                for regexpr in uninteresting:
