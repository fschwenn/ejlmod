...
publisher = 'UCLA, Los Angeles (main)'
...
for i in range(numberofpages):
    tocurl = 'https://escholarship.org/uc/ucla_etd/search?sort=desc&rows=%i&start=%i' % (articlesperpage, articlesperpage*i)
    print '---{ %i/%i }---{ %s }---'  % (i+1, numberofpages, tocurl)
    tocfilename = '/tmp/THESES-UCLA-%s.%i.%i.toc' % (stampoftoday, articlesperpage, i)
...
for rec in recs:
    i += 1
    artfilename = '/tmp/THESES-UCLA-%s' % (re.sub('\W', '', rec['artlink'][32:]))
    ...
    for meta in artpage.find_all('meta'):
        if meta.has_attr('name'):
            ...
            #author
                ...
                rec['autaff'] = [[ meta['content'], 'UCLA, Los Angeles (main)' ]]
            #year 
        ...
        if scriptt:
                ...
                if 'authors' in scripttjson['attrs'].keys():
                    for author in scripttjson['attrs']['authors']:
                        if 'email' in author.keys():
                            rec['autaff'] = [[ author['name'], 'EMAIL:%S' % (author['email']), 'UCLA, Los Angeles (main)' ]]
            if 'citation' in scripttjson.keys():
                ...
                if 'doi' in scripttjson['citation'].keys() and scripttjson['citation']['doi']:
                    ...
                    rec['doi'] = '20.2000/UCLA/' + re.sub('.*\/', '', rec['artlink'])
...
for s in  subjectrecs.keys():
    if not s in subjectstoskip:
        jnlfilename = 'THESES-UCLA-%s_%i-%s' % (stampoftoday, numberofpages*articlesperpage, s)
