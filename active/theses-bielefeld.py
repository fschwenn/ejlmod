...
numberofrecords = 200
jsonurl = 'https://pub.uni-bielefeld.de/export?cql=type%3Dbi*&cql=type%3Dbi_dissertation&fmt=json&limit=' + str(numberofrecords) + '&sort=year.desc'

print jsonurl

prerecs = {}
jfilename = '/tmp/%s.json' % (jnlfilename)
...
for thesis in theses:
    ...
    if 'ddc' in thesis.keys():
        ...
        if thesis['ddc'][0][:2] in ['50', '51', '52', '53']:          
            print '---{ %i/%i }---{ %s }---' % (i, numberofrecords, ', '.join(rec.keys()))
